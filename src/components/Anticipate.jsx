        ...
        <div className={id % 2 === 0 ? 'lg:mr-16 lg:ml-32 collapse-object lg:h-full' : 'lg:mr-32 lg:ml-16 collapse-object lg:h-full'} key={id}>
          ...
          >
            ...
            <button type='button' className='collapsible' id={`botonAnticipate${id}`}>
              <img className='arrowAnticipate h-6 w-6 lg:h-auto lg:w-8' src={arrowBottom} alt='arrowDown' />
            </button>
          </div>
          <ul className='list-disc list-outside mt-3 pl-4 montserrat-l pb-1 text-gray contenido'>
            {lista.map((row, index) => {
              const { id } = row;
              const { texto } = row;
              return (
